...
if (isset($_POST['username']) && isset($_POST['email']) && isset($_POST['password'])) {
    ...
    if (!empty(trim($_POST['username'])) && !empty(trim($_POST['email'])) && !empty($_POST['password'])) {
        ...
        $user_email = mysqli_real_escape_string($koneksi, htmlspecialchars($_POST['email']));
        ...
        if (filter_var($user_email, FILTER_VALIDATE_EMAIL)) {
            ...
            $check_email = mysqli_query($koneksi, "SELECT `email` FROM `login` WHERE email = '$user_email'");
...
-- */
                ...
                $insert_user = mysqli_query($koneksi, "INSERT INTO `login` (username, email, password) VALUES ('$username', '$user_email', '$user_hash_password')");
